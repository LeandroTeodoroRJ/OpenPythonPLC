...
WAKEUP_DELAY = 0.4
BOOT_DELAY = 1
DELAY_BETWEEN_COMMANDS = 0.020  #0.020
DELAY_BEFORE_READ = 0.01  #0.01
SERIAL_TIMEOUT = 0.015  #0.015
...
#           self.error_output_status = int(partial2)
    ...
    def encoder_update(self):
        ...
        if self.receiver != []:
            #Cast to String
            self.str_buffer_receiver = self.receiver[0].decode("utf-8")
            self.encoder_value = self.str_buffer_receiver[0:6]
            self.encoder_count = int(self.encoder_value)
            #TODO: Analize if is important create a feedback 
            #output loop error monitoring.
#           self.error_output_status = int(partial2)
        else:
    ...
    def set_step_motor_position(self, value):
        #Step Motor Update
        self.position = value + 32000
        self.msg = "20"
        self.msg = self.msg + f"{self.position: 04X}"
        self.msg = self.msg.replace(" ", "")
        self.msg = bytes(self.msg, "utf-8")
        self.ser.write(self.msg)
        time.sleep(DELAY_BETWEEN_COMMANDS)	#Be carefull with to fast time delay
...
#    print(output_info)
#    print("Encoder Value:")
#    print(dg_card1.get_encoder_value())
...
#        dg_card1.set_output_status(False, 0)
    #Step Motor Test
    time.sleep(2)
    dg_card1.set_step_motor_position(400)
    time.sleep(2)
    dg_card1.set_step_motor_position(-200)
